	...
	<body>
		...
		<main>
			...
			<div class="card">
				...
				<label>Version</label>
				<input id="up_version" placeholder="1.0.0" required />
				<label>OS</label>
				<select id="up_os">
		...
		<script>
			...
			async function uploadArtifact(){
				try {
					// 驗證輸入欄位
					const version = document.getElementById('up_version').value.trim();
					const os = document.getElementById('up_os').value.trim();
					const arch = document.getElementById('up_arch').value.trim();
					const fileInput = document.getElementById('up_file');
					
					if (!version) { alert('請輸入版本號'); return; }
					if (!os) { alert('請選擇作業系統'); return; }
					if (!arch) { alert('請選擇架構'); return; }
					if (!fileInput.files.length) { alert('請選擇檔案'); return; }
					
					// 顯示上傳進度
					document.getElementById('up_result').textContent = '正在上傳...';
					...
					console.log('上傳參數:', { channel, version, os, arch, filename: fileInput.files[0].name });
					...
					if(res.ok){ 
						...
						fetchManifests(); 
						// 清空表單
						document.getElementById('up_version').value = '';
						document.getElementById('up_file').value = '';
					} else {
						alert('上傳失敗: ' + (j.error || '未知錯誤'));
					}
				} catch(e){ 
					console.error('uploadArtifact error', e);
					document.getElementById('up_result').textContent = '錯誤: ' + e.message;
				}
			}
			loadConfig();
			fetchManifests();
