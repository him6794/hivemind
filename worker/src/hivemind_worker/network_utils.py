...
def _pick_preferred_ip_from_interfaces(node, interfaces_list: list[str]) -> Optional[str]:
    """Pick a preferred IPv4 from available interfaces.

    Preference order:
    1) Any 10.0.0.* address (VPN subnet requested by product requirement)
    2) WireGuard-like interface address (wg/wireguard)

    Returns None if nothing suitable is found.
    """
    # 1) Prefer 10.0.0.* anywhere
            ...
            if AF_INET in addrs:
                for addr_info in addrs[AF_INET]:
                    ...
                    if ip.startswith(essential_vpn_prefix) and ip != f'{essential_vpn_prefix}1':
                        node._log(f"Detected preferred VPN subnet IP: {ip} (interface: {iface})")
                        return ip
        except Exception:
            continue

    # 2) Then prefer WireGuard-like interfaces
    wg_interfaces = [
        iface for iface in interfaces_list
    ...
    for wg_iface in wg_interfaces:
        try:
            ...
            if AF_INET in addrs:
                wg_ip = addrs[AF_INET][0].get('addr')
                if wg_ip:
...
def get_local_ip(node) -> str:
    try:
        ...
        preferred = _pick_preferred_ip_from_interfaces(node, interfaces_list)
        if preferred:
            return preferred
    except Exception as e:
