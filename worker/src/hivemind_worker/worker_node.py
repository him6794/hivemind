...
# 支援「作為套件」與「直接執行」兩種導入模式
try:
    # 嘗試相對導入 (作為包運行時)
    from .config import NODE_PORT, NODEPOOL_ADDRESS, NODE_ID
...
try:
    from .credential_store import save_credentials, load_credentials, delete_credentials, has_saved_credentials
except ImportError:
    from credential_store import save_credentials, load_credentials, delete_credentials, has_saved_credentials
...
class WorkerNode:
    def __init__(self):
        ...
        self.port = NODE_PORT
        # Address of the node pool service this worker connects to
        self.nodepool_address = NODEPOOL_ADDRESS
        # Flask port 由系統自動分配 (使用 0 讓系統選擇可用端口)
        import random
        self.flask_port = random.randint(5050, 6000)
        ...
        self._init_docker()
        # gRPC 連接
        self._init_grpc()  # 確保 gRPC stub 初始化在 Flask 之前，連線到 nodepool
        # Flask 應用
        self._init_flask()
    ...
    def _init_auto_services(self):
        ...
        try:
            ...
            try:
                ...
                self._log(f"AutoUpdate module not available: {e}", WARNING)
                return
        ...
        try:
            ...
            self._log(f"Auto-update initialization failed: {e}", WARNING)
    ...
    def _init_hardware(self):
        ...
        try:
            ...
            # 使用 C 共享庫進行實際基準測試（GOPS）
            try:
                from .performance_calculator_v3 import calculate_performance
                perf = calculate_performance(quick_cpu=True)
                self.cpu_score = perf['cpu_tops']  # GOPS
                self.gpu_score = perf['gpu_tops']  # GOPS
                self.gpu_name = perf['gpu_name']
                self.gpu_memory_gb = perf['gpu_memory_gb']
                self._log(
                    f"Performance (Benchmark): CPU={self.cpu_score} GOPS, GPU={self.gpu_score} GOPS, Total={perf['total_tops']} GOPS"
                )
            except Exception as e:
                self._log(f"Benchmark failed, using fallback: {e}", WARNING)
            ...
            self._log(
                f"Hardware: CPU={self.cpu_cores} cores, RAM={self.memory_gb:.1f}GB available (Total: {self.total_memory_gb:.1f}GB), "
                f"GPU={self.gpu_name} ({self.gpu_memory_gb}GB)"
            )
    ...
    def _try_auto_login(self):
        ...
        try:
            ...
            username, password, remember = load_credentials()
            if not (username and password and remember):
                # saved username might be shown in UI even if password missing; do not attempt auto-login
                self._log("Saved credentials incomplete, skipping auto-login", WARNING)
                return

            self._log(f"Found saved credentials for {username}, attempting auto-login...")

            # Ensure gRPC is ready; if not, try to initialize and retry a few times
            max_attempts = 3
            attempt = 0
            while attempt < max_attempts:
                attempt += 1
                # If connection not ready, try to re-init
                if not getattr(self, 'user_stub', None):
                    try:
                        self._init_grpc()
                    except Exception:
                        pass

                ok = self._login(username, password, remember=True, auto_login=True)
                if ok:
                    self._log(f"Auto-login successful for {username}")
                    # 自動登入成功後嘗試註冊
                    try:
                        if self._register():
                            self._log(f"Auto-registration successful for {username}")
                        else:
                            self._log(f"Auto-registration failed for {username}", WARNING)
                    except Exception as e:
                        self._log(f"Auto-registration error: {e}", WARNING)
                    return

                # 如果失敗，根據狀態判斷是否為連線問題或憑證錯誤
                status = getattr(self, 'status', '') or ''
                # 若為連線問題，稍後重試；若為憑證/密碼錯誤，刪除憑證並停止
                if 'No Connection' in status or 'Connection' in status:
                    self._log(f"Auto-login attempt {attempt} failed due to connection ({status}), retrying...", WARNING)
                    time.sleep(2 * attempt)
                    continue
                else:
                    # 其他錯誤通常表示憑證無效，不要保留會導致 UI 顯示但無法登入
                    self._log(f"Auto-login failed for {username} due to invalid credentials or server rejection ({status}), clearing saved credentials", WARNING)
                    try:
                        delete_credentials()
                    except Exception:
                        pass
            ...
            # 如果嘗試多次仍失敗且為連線問題，保留憑證但記錄警告
            self._log(f"Auto-login aborted after {max_attempts} attempts (likely network issue). Credentials retained for manual retry.", WARNING)
        except Exception as e:
            self._log(f"Auto-login error: {e}", WARNING)

    def _login(self, username, password, remember=False, auto_login=False):
        """登入到節點池"""
        try:
            # 檢查 gRPC 連接是否可用
            ...
            if response.success and response.token:
                ...
                # 儲存憑證（如果用戶選擇記住）
                if remember and not auto_login:
                    try:
                        save_credentials(username, password, remember=True)
                        self._log(f"Credentials saved for {username}")
                    except Exception as e:
                        self._log(f"Failed to save credentials: {e}", WARNING)
    ...
    def _register(self):
        ...
        try:
            ...
            self._log(f"Attempting to register node with nodepool at {getattr(self, 'nodepool_address', None)}")
            self._log(
                f"Registration - 硬體總效能: CPU={int(self.cpu_score)}, GPU={int(self.gpu_score)}, RAM={int(self.total_memory_gb)}GB"
            )
            self._log(
                f"Registration - 廣告效能（受限制）: CPU={int(self.advertised_cpu_score)}, "
                f"GPU={int(self.advertised_gpu_score)}, RAM={int(self.advertised_memory_gb)}GB, "
                f"GPU_MEM={int(self.advertised_gpu_memory_gb)}GB"
            )
            self._log(f"Registration - 網路: IP={self.local_ip}, Port={self.port}, User={self.username}")
            ...
            request = nodepool_pb2.RegisterWorkerNodeRequest(
                ...
                memory_gb=int(self.advertised_memory_gb),       # 使用廣告值
                cpu_score=int(self.advertised_cpu_score),       # 使用廣告值
                gpu_score=int(self.advertised_gpu_score),       # 使用廣告值
                gpu_name=self.gpu_name,
                gpu_memory_gb=int(self.advertised_gpu_memory_gb),  # 使用廣告值
            ...
            if response.success:
                ...
                self._log(f"已註冊廣告資源: CPU={self.advertised_cpu_score}, GPU={self.advertised_gpu_score}")
                return True
            else:
    ...
    def _refresh_registration(self):
        """刷新註冊以維持心跳，使用廣告資源值（受效能限制影響）"""
        if not self.is_registered or not self.node_stub:
            return False
            
        try:
            # 心跳刷新使用廣告資源值（受 performance_limits 影響）
            request = nodepool_pb2.RegisterWorkerNodeRequest(
                node_id=self.username,
                hostname=self.local_ip,
                cpu_cores=int(self.cpu_cores),
            ...
            response = self.node_stub.RegisterWorkerNode(request, metadata=metadata, timeout=5)
            
            if response.success:
                self._log(
                    f"心跳更新成功 - 廣告資源: CPU={self.advertised_cpu_score}, "
                    f"GPU={self.advertised_gpu_score}, Memory={self.advertised_memory_gb}GB"
                )
    ...
    def _logout(self):
        ...
        try:
            ...
            self._log(f"Logout post-cleanup error: {e}", WARNING)
    ...
    def _cleanup_docker_containers(self):
        """清理所有 HiveMind Docker 容器"""
        try:
            try:
                from .docker_utils import cleanup_old_containers
            except ImportError:
                from docker_utils import cleanup_old_containers
            cleanup_old_containers(self)
        except Exception as e:
            self._log(f"Failed to cleanup docker containers: {e}", level=30)
...
def run_worker_node():
    ...
    try:
        ...
        print(f"NodePool Address: {NODEPOOL_ADDRESS}")
        
        # 創建 Worker 節點實例
        worker = WorkerNode()
        
        # 啟動 gRPC 服務器 - 設定大檔案傳輸限制
        grpc_options = [
        ...
        print(f"Worker node web interface will be available at http://127.0.0.1:{worker.flask_port}")
        ...
        try:
            ...
            # 清理 Docker 容器
            worker._cleanup_docker_containers()
            server.stop(5)
