document.addEventListener('DOMContentLoaded', function(){
    ...
    function updateTasksGrid(tasks){
        ...
        tasks.forEach(task=>{
            ...
            const cpuPct = (usage.cpu_percent != null) ? Number(usage.cpu_percent) : null;
            const memPct = (usage.memory_percent != null) ? Number(usage.memory_percent) : null;
            const gpuPct = (usage.gpu_percent != null) ? Number(usage.gpu_percent) : null;

            const memUsedMb = (usage.memory_used_mb != null) ? Number(usage.memory_used_mb) : null;
            const memLimitMb = (usage.memory_limit_mb != null) ? Number(usage.memory_limit_mb) : null;
            const memExtra = (memUsedMb != null && memLimitMb != null && memLimitMb > 0)
                ? ` (${(memUsedMb/1024).toFixed(2)}/${(memLimitMb/1024).toFixed(2)} GB)`
                : '';

            const clampPct = (v) => Math.min(100, Math.max(0, v));
            const cpuBar = (cpuPct == null || Number.isNaN(cpuPct)) ? 0 : clampPct(cpuPct);
            const memBar = (memPct == null || Number.isNaN(memPct)) ? 0 : clampPct(memPct);
            const gpuBar = (gpuPct == null || Number.isNaN(gpuPct)) ? 0 : clampPct(gpuPct);

            const cpuLabel = (cpuPct == null || Number.isNaN(cpuPct)) ? (res.cpu ?? 0) : `${cpuPct.toFixed(1)}%`;
            const memLabel = (memPct == null || Number.isNaN(memPct)) ? `${res.memory_gb ?? 0} GB` : `${memPct.toFixed(1)}%${memExtra}`;
            const gpuLabel = (gpuPct == null || Number.isNaN(gpuPct)) ? (res.gpu ?? 0) : `${gpuPct.toFixed(1)}%`;
            ...
            card.innerHTML = `
                ...
                <div class="resource-bar">
                    <div class="resource-label"><span>CPU</span><span>${cpuLabel}</span></div>
                    <div class="progress-bar"><div class="progress-fill low" style="width:${cpuBar}%"></div></div>
                </div>
                <div class="resource-bar">
                    <div class="resource-label"><span>MEM</span><span>${memLabel}</span></div>
                    <div class="progress-bar"><div class="progress-fill medium" style="width:${memBar}%"></div></div>
                </div>
                <div class="resource-bar">
                    <div class="resource-label"><span>GPU</span><span>${gpuLabel}</span></div>
                    <div class="progress-bar"><div class="progress-fill high" style="width:${gpuBar}%"></div></div>
                </div>
