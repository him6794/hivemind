...
try:
    from .credential_store import load_credentials, has_saved_credentials  # type: ignore
except Exception:
    from credential_store import load_credentials, has_saved_credentials  # type: ignore
...
def register_routes(app, worker):
    ...
    def index():
        ...
        try:
            if has_saved_credentials():
                username, _, remember = load_credentials()
    ...
    def login_route():
        if request.method == 'GET':
            ...
            # 如果伺服器端已經有登錄的使用者（例如在程式啟動時自動登入），
            # 但瀏覽器尚未持有 session cookie，則在此為該使用者建立會話並導向 monitor。
            if user_data and user_data['username'] == worker.username:
                return redirect(url_for('monitor'))

            if not user_data and getattr(worker, 'username', None):
                try:
                    # 建立使用者會話並將 session_id 存入瀏覽器 cookie
                    new_sid = worker._create_user_session(worker.username, worker.token)
        ...
        remember = request.form.get('remember') == 'on'  # 檢查是否勾選記住密碼
        ...
        if worker._login(username, password, remember=remember):
            # 登入成功，現在嘗試註冊
    ...
    def api_metrics():
        ...
        # Network throughput (Mbps) - psutil removed; not available
        rx_mbps = tx_mbps = 0.0
