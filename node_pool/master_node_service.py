...
class FileStorageManager:
    ...
    def get_result_zip_path(self, task_id):
        task_id = self._validate_task_id(task_id)
        file_path = os.path.join(self.base_path, f"result_{task_id}.zip")
        base = os.path.abspath(self.base_path)
        full = os.path.abspath(file_path)
        if not (full == base or full.startswith(base + os.sep)):
...
class TaskManager:
    ...
    def store_task(self, task_id, task_zip, memory_gb, cpu_score, gpu_score, gpu_memory_gb, location, gpu_name, user_identifier, cpt_cost=None):
        """存儲任務 - user_identifier 可以是 username 或 user_id"""
        task_key = f"task:{task_id}"
        try:
            # 統一轉換為 user_id（整數）和 username（字串）
            if isinstance(user_identifier, str) and not user_identifier.isdigit():
                # 傳入的是 username
                username = user_identifier
                user_info = self.db_manager.get_user_by_username(username)
                if not user_info:
                    logging.error(f"用戶 {username} 不存在")
                    return False
                ...
                if not user_info:
                    logging.error(f"用戶 ID {user_id} 不存在")
                    return False
                username = user_info['username']
            
            logging.info(f"存儲任務 {task_id} 的詳細信息: memory_gb={memory_gb}, cpu_score={cpu_score}, "
                         f"gpu_score={gpu_score}, gpu_memory_gb={gpu_memory_gb}, user_id={user_id}, username={username}")
            ...
            task_info = {
                ...
                "output": "",
                "assigned_node": "",
                "user_id": str(user_id),         # 統一存儲整數 ID
                "username": username,            # 額外存儲 username 便於顯示
    ...
    def update_task_status(self, task_id, status, assigned_node=None):
        """更新任務狀態，可選地更新分配節點，並為完成/失敗的任務設置 TTL"""
        task_key = f"task:{task_id}"
        try:
            update_data = {"status": status, "updated_at": str(time.time())}
            ...
            # 為已完成、失敗或停止的任務設置 TTL（Redis 過期時間作為雙重保險）
            if status in ["COMPLETED", "FAILED", "STOPPED"]:
                from config import Config
                retention_hours = getattr(Config, 'TASK_RETENTION_HOURS', 24)
                ttl_seconds = retention_hours * 3600
                self.redis_client.expire(task_key, ttl_seconds)
                logging.info(f"任務 {task_id} 狀態更新為 {status}，已設置 {retention_hours} 小時 TTL")
    ...
    def get_task_info(self, task_id, include_zip=False):
        ...
        string_keys = [
            ...
            "user_id", "username", "created_at", "updated_at", "logs", "cpt_cost",
    ...
    def force_stop_task(self, task_id):
        ...
        try:
            ...
            if not task_info:
                logging.warning(f"任務 {task_id} 不存在，無法強制停止")
                return False

            assigned_node = task_info.get("assigned_node")
            if assigned_node:
                # 調用 NodeManager 的 stop_task 方法
                success, message = self.node_manager.stop_task(assigned_node, task_id)
                if not success:
                    logging.error(f"無法停止節點 {assigned_node} 上的任務 {task_id}: {message}")
                    return False
            ...
            logging.error(f"強制停止任務 {task_id} 時發生錯誤: {e}", exc_info=True)
            return False
...
class MasterNodeServiceServicer(nodepool_pb2_grpc.MasterNodeServiceServicer):
                ...
                try:
                    ...
                    for task in running_tasks:
                        ...
                        if current_status in ("STOPPED", "COMPLETED", "FAILED"):
                            logging.debug(f"任務 {task_id} 狀態已終結 ({current_status})，跳過健康檢查")
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    for task in running_tasks:
                        ...
                        last_charge_time_str = self.task_manager.redis_client.get(last_charge_key)
                        
                        if last_charge_time_str:
                            last_charge_time = int(float(last_charge_time_str))
                            # 如果距離上次扣費不足 50 秒，跳過（考慮調度器間隔）
                            if current_time - last_charge_time < 50:
                                logging.debug(f"任務 {task_id} 上次扣費時間 {last_charge_time}，距今 {current_time - last_charge_time}秒，跳過重複扣費")
                                continue
                        ...
                        if user_id and assigned_node:
                            ...
                            logging.debug(f"任務 {task_id} 使用固定費用: {final_cost} CPT/分鐘，user_id={user_id}, assigned_node={assigned_node}")
                            
                            # user_id 可能是數字 ID 或 username，檢查並轉換
                            # check_user_balance_for_next_payment 和 transfer_tokens 都接受 user_identifier（可以是 ID 或 username）
                            ...
                            if success:
                                # === 扣費成功後立即記錄時間（冪等性保證）===
                                self.task_manager.redis_client.setex(
                                    last_charge_key,
                                    120,  # 保留 2 分鐘（超過調度間隔）
                                    str(current_time)
                                )
                                
                                logging.info(f"任務 {task_id} 轉帳成功: {final_cost} CPT 從發起者 {user_id} 到工作端 {assigned_node} (時間: {current_time})")
    ...
    def _increment_task_fail_count(self, task_id, assigned_node, user_id):
        """增加任務失敗計數並處理任務失敗邏輯"""
        try:
            # 若任務已是 COMPLETED，不可覆蓋為 FAILED（避免 race condition）
            current_info = self.task_manager.get_task_info(task_id, include_zip=False)
            if current_info and current_info.get("status") == "COMPLETED":
                logging.info(f"任務 {task_id} 已是 COMPLETED，不覆蓋為 FAILED，清除 task_health")
                if task_id in self.task_health:
                    del self.task_health[task_id]
                return
            # 將任務標記為失敗
    ...
    def StopTask(self, request, context):
        ...
        try:
            ...
            # 檢查任務所有權（直接比較 username）
            task_username = task_info.get("username", "")
            logging.info(f"任務 {task_id} 的擁有者: {task_username}, 請求停止的用戶: {username}")
            
            if task_username != username:
                logging.warning(f"用戶 {username} 無權限停止任務 {task_id}（所有者: {task_username}）")
    ...
    def GetTaskResult(self, request, context):
            ...
            try:
                ...
                if hasattr(request, 'token') and request.token:
                    ...
                    # 檢查任務是否屬於該用戶
                    task_info = self.task_manager.get_task_info(task_id, include_zip=False)
                    if task_info and task_info.get("username") != username:
                        logging.warning(f"用戶 {username} 嘗試下載任務 {task_id}（所有者: {task_info.get('username')}）")
    ...
    def GetTasklog(self, request, context):
        ...
        try:
            ...
            task_info = self.task_manager.get_task_info(task_id, include_zip=False)
            if not task_info:
                logging.warning(f"GetTasklog: 任務 {task_id} 不存在")
                context.set_code(grpc.StatusCode.NOT_FOUND)
                context.set_details("Task not found")
                return nodepool_pb2.TasklogResponse(success=False, log="")

            # 比較任務的 username 和請求者的 username
            task_username = task_info.get("username", "")
            logging.info(f"GetTasklog: 任務 {task_id} 所有者='{task_username}', 請求者='{username}'")
            
            if task_username != username:
                logging.warning(f"用戶 {username} 嘗試訪問任務 {task_id}（所有者: {task_username}）")
                logging.warning(f"  task_username type: {type(task_username)}, value: {repr(task_username)}")
                logging.warning(f"  username type: {type(username)}, value: {repr(username)}")
                logging.warning(f"  比較結果: {task_username} != {username} = {task_username != username}")
                context.set_code(grpc.StatusCode.PERMISSION_DENIED)
    ...
    def UploadTask(self, request, context):
        ...
        try:
            ...
            if file_size_mb > 500:
                ...
                return nodepool_pb2.UploadTaskResponse(
                    success=False,
                    message=f"任務文件過大: {file_size_mb:.1f}MB (限制: 500MB)"
                )
            ...
            success = self.task_manager.store_task(
                ...
                location=location,
                gpu_name=gpu_name,
                user_identifier=username  # 使用正確的參數名
            )
    ...
    def GetTaskResult(self, request, context):
        ...
        try:
            ...
            if task_info.get("username") != username:
                logging.warning(f"用戶 {username} 嘗試訪問任務 {task_id}（所有者: {task_info.get('username')}）")
                return nodepool_pb2.GetTaskResultResponse(
                    success=False,
                    message="無權限訪問此任務",
                    result_zip=b""
        ...
        try:
            ...
            try:
                ...
                for key in task_keys:
                    ...
                    task_id = key.split(":", 1)[1]
                    task_username = self.task_manager.redis_client.hget(key, "username")
                    
                    # 比較用戶名
                    if task_username == username:
                        status = self.task_manager.redis_client.hget(key, "status") or "UNKNOWN"
                        message = self.task_manager.redis_client.hget(key, "message") or ""
                        # 以浮點数解析使用率，兼容字串格式，例如 "7.34"
                        def _to_float(v):
                            try:
                                return float(v)
                            except Exception:
