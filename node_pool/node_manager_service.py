...
class NodeManagerServiceServicer(nodepool_pb2_grpc.NodeManagerServiceServicer):
    ...
    def UploadTask(self, request, context):
        """處理用戶上傳任務請求"""
        logging.info(f"收到任務上傳請求: {request.task_id}, 需要節點數: {request.host_count}")
        try:
            host_count = request.host_count
            if host_count < 0:
                return nodepool_pb2.UploadTaskResponse(success=False, message="host_count 不能為負數")

            # 獲取可用節點列表
            available_nodes = self.node_manager.get_available_nodes()

            if host_count > 0 and len(available_nodes) < host_count:
                return nodepool_pb2.UploadTaskResponse(success=False, message="可用節點數不足")
            ...
            for node in selected_nodes:
                self.node_manager.assign_task_to_node(node, request.task_id, request.task_zip)

            logging.info(f"任務 {request.task_id} 已分發到節點: {[node['node_id'] for node in selected_nodes]}")
            return nodepool_pb2.UploadTaskResponse(success=True, message="任務分發成功")

        except Exception as e:
            logging.error(f"處理任務 {request.task_id} 上傳請求時發生錯誤: {e}", exc_info=True)
            return nodepool_pb2.UploadTaskResponse(success=False, message=f"任務分發錯誤: {str(e)}")