...
pub async fn find_best_worker(task: &Task, workers: &[WorkerNode]) -> Option<WorkerNode> {
    let mut candidates: Vec<&WorkerNode> = workers
        ...
        .filter(|w| {
            matches!(w.status, WorkerStatus::Active | WorkerStatus::Idle)
                ...
                && w.storage_available_gb >= task.req_storage_gb
        })
        .collect();
    ...
    candidates.sort_by(|a, b| {
        fn status_priority(s: &WorkerStatus) -> u8 {
            match s { WorkerStatus::Idle => 0, WorkerStatus::Active => 1, WorkerStatus::Busy => 2, _ => 3 }
        }
        status_priority(&a.status).cmp(&status_priority(&b.status))
            .then_with(|| a.cpu_usage.partial_cmp(&b.cpu_usage).unwrap_or(std::cmp::Ordering::Equal))
            .then_with(|| b.available_memory_gb.cmp(&a.available_memory_gb))
            .then_with(|| b.storage_available_gb.cmp(&a.storage_available_gb))
...
mod tests {
    ...
    fn make_worker(id: &str, cpu: i32, mem: i32, cpu_usage: f64, status: WorkerStatus) -> WorkerNode {
        WorkerNode {
            ...
            ip: "127.0.0.1".into(), virtual_ip: None, hostname: None,
            ...
            cpu_usage, memory_usage: 0.0, gpu_usage: 0.0, gpu_memory_usage: 0.0,
    ...
    async fn test_find_best_worker_prefers_idle() {
        ...
        let task = Task {
            id: uuid::Uuid::new_v4(), task_id: "t1".into(), owner: "u1".into(),
            worker_id: None, worker_ip: None,
            status: hivemind_models::TaskStatus::Pending, status_message: None,
            output: None, result_torrent: None,
            torrent_source: Some("btih".into()), expected_btih: None,
            cpu_usage: 0.0, memory_usage: 0.0, gpu_usage: 0.0, gpu_memory_usage: 0.0,
            req_cpu_score: 300, req_gpu_score: 0, req_memory_gb: 8, req_gpu_memory_gb: 0,
            req_storage_gb: 10,
            ...
            created_at: chrono::Utc::now(), last_update: chrono::Utc::now(), completed_at: None,
        };
    ...
    async fn test_no_suitable_worker() {
        ...
        let task = Task {
            id: uuid::Uuid::new_v4(), task_id: "t2".into(), owner: "u1".into(),
            worker_id: None, worker_ip: None,
            status: hivemind_models::TaskStatus::Pending, status_message: None,
            output: None, result_torrent: None,
            torrent_source: Some("btih".into()), expected_btih: None,
            cpu_usage: 0.0, memory_usage: 0.0, gpu_usage: 0.0, gpu_memory_usage: 0.0,
            req_cpu_score: 500, req_gpu_score: 0, req_memory_gb: 32, req_gpu_memory_gb: 0,
            req_storage_gb: 10,
            host_count: 1, max_cpt: 1000,
            billing_settled: false, billed_amount: 0,
            retry_count: 0, max_retries: 3, deadline: None,
