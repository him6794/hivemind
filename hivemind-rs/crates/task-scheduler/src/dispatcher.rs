...
use hivemind_models::{Task, WorkerNode, TaskStatus, WorkerStatus};
use std::sync::Arc;
...
impl Dispatcher {
    pub fn new(db: DatabaseManager, task_timeout_secs: u64, max_redispatch: i32) -> Self {
        Self { repo: Arc::new(TaskRepository::new(db.pool.clone())), db, task_timeout_secs, max_redispatch }
    }

    pub async fn dispatch_one(&self, task: &Task, workers: &[WorkerNode]) -> Option<(String, String)> {
        ...
        match self.repo.assign_to_worker(&task.task_id, &wid, &wip).await {
            Ok(_) => { info!("Dispatched task {} to worker {} ({})", task.task_id, wid, wip); Some((wid, wip)) }
            Err(e) => { error!("Failed to dispatch task {}: {}", task.task_id, e); None }
        }
    }
    ...
    pub async fn dispatch_pending(&self, workers: &[WorkerNode]) -> Result<u64> {
        ...
        for task in &pending {
            if self.dispatch_one(task, workers).await.is_some() { dispatched += 1; }
        }
        if dispatched > 0 { info!("Dispatched {} pending tasks", dispatched); }
        Ok(dispatched)
    }

    pub async fn process_timeouts(&self) -> Result<(u64, u64)> {
        let stale = self.repo.find_stale_dispatched(self.task_timeout_secs).await?;
        ...
        for task in &stale {
            if task.retry_count >= self.max_redispatch {
                match self.repo.fail(&task.task_id, "Max redispatch attempts exceeded").await {
                    Ok(_) => { warn!("Task {} failed after {} retries", task.task_id, task.retry_count); failed += 1; }
                    Err(e) => error!("Failed to mark task {} as failed: {}", task.task_id, e),
                }
            } else {
                match self.repo.reset_to_pending(&task.task_id).await {
                    Ok(_) => { info!("Task {} reset to pending (retry {}/{})", task.task_id, task.retry_count + 1, self.max_redispatch); redispatched += 1; }
                    Err(e) => error!("Failed to reset task {}: {}", task.task_id, e),
                }
            }
        }
        let timed_out = self.repo.mark_stale_running().await?;
        if timed_out > 0 { warn!("Marked {} running tasks as timed out", timed_out); }
        Ok((redispatched, failed))
    }

    pub fn start_dispatch_loop(self: Arc<Self>, workers_rx: watch::Receiver<Vec<WorkerNode>>, interval: std::time::Duration) -> watch::Sender<bool> {
    ...
    pub fn start_timeout_loop(self: Arc<Self>, interval: std::time::Duration) -> watch::Sender<bool> {
        let (tx, mut rx) = watch::channel(false);
        tokio::spawn(async move {
            let mut tick = tokio::time::interval(interval);
            loop {
...
mod tests {
    ...
    fn make_task(id: &str, status: TaskStatus, retry_count: i32) -> Task {
        Task {
            id: uuid::Uuid::new_v4(), task_id: id.into(), owner: "testuser".into(),
            worker_id: None, worker_ip: None, status,
            status_message: None, output: None, result_torrent: None,
            torrent_source: Some("test-btih".into()), expected_btih: None,
            cpu_usage: 0.0, memory_usage: 0.0, gpu_usage: 0.0, gpu_memory_usage: 0.0,
            req_cpu_score: 100, req_gpu_score: 0, req_memory_gb: 4, req_gpu_memory_gb: 0,
            req_storage_gb: 10,
            host_count: 1, max_cpt: 1000, billing_settled: false, billed_amount: 0,
            retry_count, max_retries: 3, deadline: None,
    ...
    fn make_worker(id: &str, cpu: i32, mem: i32, status: WorkerStatus) -> WorkerNode {
        WorkerNode {
            id: uuid::Uuid::new_v4(), worker_id: id.into(), username: "test".into(),
            ip: format!("192.168.1.{}", &id[1..]), virtual_ip: None, hostname: None,
            cpu_cores: cpu, memory_gb: mem, cpu_score: cpu * 100,
            gpu_score: 0, gpu_memory_gb: 0, gpu_name: None, vram_mb: 0,
            storage_total_gb: 500, storage_available_gb: 200,
            location: "local".into(), status,
            cpu_usage: 0.0, memory_usage: 0.0, gpu_usage: 0.0, gpu_memory_usage: 0.0,
            available_memory_gb: mem, queue_capacity: cpu,
            last_heartbeat: Utc::now(), registered_at: Utc::now(), updated_at: Utc::now(),
        }
    }

    #[test]
    fn test_dispatcher_creation() { assert_eq!(30u64, 30); }

    #[tokio::test]
    async fn test_dispatch_one_no_workers() {
        let config = hivemind_config::HivemindConfig::default();
        let db = match hivemind_database::DatabaseManager::new(&config).await { Ok(db) => db, Err(_) => return };
        ...
        sqlx::query("DELETE FROM tasks WHERE task_id = 'dispatch-test-1'").execute(&dispatcher.db.pool).await.ok();
    }
    ...
    async fn test_dispatch_one_with_worker() {
        ...
        sqlx::query("DELETE FROM tasks WHERE task_id = 'dispatch-test-2'").execute(&db.pool).await.ok();
    }
    ...
    async fn test_dispatch_pending_multiple() {
        ...
        let workers = vec![make_worker("wm1", 8, 32, WorkerStatus::Idle), make_worker("wm2", 8, 32, WorkerStatus::Idle)];
        let count = dispatcher.dispatch_pending(&workers).await.unwrap();
        assert!(count >= 1);
        for i in 0..3 {
            sqlx::query("DELETE FROM tasks WHERE task_id = ").bind(format!("dispatch-multi-{}", i)).execute(&db.pool).await.ok();
        }
    }
}
