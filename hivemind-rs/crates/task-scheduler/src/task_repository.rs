...
impl TaskRepository {
    ...
    pub async fn create(&self, task: &Task) -> Result<Task> {
        let created = sqlx::query_as::<_, Task>(
            r#"
            INSERT INTO tasks (
                task_id, owner, status, status_message,
                torrent_source, expected_btih,
                req_cpu_score, req_gpu_score, req_memory_gb, req_gpu_memory_gb,
                host_count, max_cpt, max_retries, deadline,
                deterministic, side_effects, priority,
        ...
        .bind(&task.torrent_source)
        .bind(&task.expected_btih)
        .bind(task.req_cpu_score)
        .bind(task.req_gpu_score)
        .bind(task.req_memory_gb)
        .bind(task.req_gpu_memory_gb)
        .bind(task.host_count)
        .bind(task.max_cpt)
        .bind(task.max_retries)
        .bind(task.deadline)
        .bind(task.deterministic)
        .bind(task.side_effects)
        .bind(task.priority)
        .fetch_one(&self.pool)
        .await?;
        Ok(created)
    ...
    pub async fn update_status(
        ...
        let updated = sqlx::query_as::<_, Task>(
            r#"
            UPDATE tasks SET
                status = $1, status_message = $2, last_update = NOW()
            WHERE task_id = $3
            RETURNING *
            "#,
        )
    ...
    pub async fn assign_to_worker(
        ...
        let updated = sqlx::query_as::<_, Task>(
            r#"
            UPDATE tasks SET
                worker_id = $1, worker_ip = $2, status = 'ASSIGNED',
                last_update = NOW()
            WHERE task_id = $3
            RETURNING *
            "#,
    ...
    pub async fn complete(
        ...
        let updated = sqlx::query_as::<_, Task>(
            r#"
            UPDATE tasks SET
                status = 'COMPLETED', result_torrent = $1, output = $2,
    ...
    pub async fn fail(&self, task_id: &str, reason: &str) -> Result<Task> {
        let updated = sqlx::query_as::<_, Task>(
            r#"
            UPDATE tasks SET
                status = 'FAILED', status_message = $1, last_update = NOW(), completed_at = NOW()
            WHERE task_id = $2
            RETURNING *
            "#,
        )
    ...
    pub async fn cancel(&self, task_id: &str) -> Result<Task> {
        let updated = sqlx::query_as::<_, Task>(
            r#"
            UPDATE tasks SET
                status = 'CANCELLED', last_update = NOW(), completed_at = NOW()
            WHERE task_id = $1
            RETURNING *
            "#,
        )
    ...
    pub async fn update_resource_usage(
        ...
        sqlx::query(
            "UPDATE tasks SET cpu_usage = $1, memory_usage = $2, gpu_usage = $3, gpu_memory_usage = $4, last_update = NOW() WHERE task_id = $5",
        )
        .bind(cpu)
        .bind(memory)
        .bind(gpu)
...
mod tests {
    ...
    async fn test_create_and_find_task() {
        let p = pool().await;
        if p.is_none() { return; }
        let repo = TaskRepository::new(p.unwrap());
        ...
        let task = Task {
            ...
            output: None,
            result_torrent: None,
            torrent_source: Some("fake-btih-abc123".into()),
            expected_btih: None,
            cpu_usage: 0.0,
            memory_usage: 0.0,
            gpu_usage: 0.0,
