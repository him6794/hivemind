...
impl HeartbeatHandler {
    pub fn new(manager: Arc<NodeManager>, stale_threshold_secs: u64) -> Self {
        Self { manager, stale_threshold_secs }
    }

    pub async fn process_heartbeat(
        &self, worker_id: &str, status: &str,
        cpu_usage: f64, memory_usage: f64,
        gpu_usage: f64, gpu_memory_usage: f64,
    ) -> Result<()> {
        if worker_id.trim().is_empty() { anyhow::bail!("worker_id cannot be empty"); }
        if !(0.0..=100.0).contains(&cpu_usage) { anyhow::bail!("cpu_usage must be 0-100, got {}", cpu_usage); }
        if !(0.0..=100.0).contains(&memory_usage) { anyhow::bail!("memory_usage must be 0-100, got {}", memory_usage); }

        let normalized_status = match status.to_uppercase().as_str() {
            "ACTIVE" | "IDLE" | "BUSY" | "OFFLINE" | "ERROR" => status.to_uppercase(),
            _ => { warn!("Unknown status '{}' from worker {}, defaulting to ACTIVE", status, worker_id); "ACTIVE".to_string() }
        };

        self.manager.update_heartbeat(worker_id, &normalized_status, cpu_usage, memory_usage, gpu_usage, gpu_memory_usage).await
    }

    pub fn start_cleanup_loop(self: Arc<Self>, interval: std::time::Duration) -> watch::Sender<bool> {
...
mod tests {
    ...
    async fn test_process_heartbeat_invalid() {
        let (config, db) = match setup_db().await { Some(v) => v, None => return };
        let manager = Arc::new(NodeManager::new(&config, db));
        let handler = HeartbeatHandler::new(manager, 30);
        assert!(handler.process_heartbeat("", "ACTIVE", 50.0, 50.0, 0.0, 0.0).await.is_err());
        assert!(handler.process_heartbeat("w1", "ACTIVE", 150.0, 50.0, 0.0, 0.0).await.is_err());
    }
    ...
    async fn test_process_heartbeat_valid() {
        ...
        let worker = WorkerNode {
            id: uuid::Uuid::new_v4(), worker_id: "heartbeat-test-w1".into(),
            username: "test".into(), ip: "10.0.0.1".into(),
            virtual_ip: None, hostname: None,
            cpu_cores: 4, memory_gb: 16, cpu_score: 400,
            gpu_score: 0, gpu_memory_gb: 0,
            gpu_name: None, vram_mb: 0,
            storage_total_gb: 500, storage_available_gb: 200,
            location: "test".into(), status: WorkerStatus::Active,
            cpu_usage: 0.0, memory_usage: 0.0, gpu_usage: 0.0, gpu_memory_usage: 0.0,
            available_memory_gb: 16, queue_capacity: 4,
            last_heartbeat: chrono::Utc::now(),
            registered_at: chrono::Utc::now(), updated_at: chrono::Utc::now(),
        };
