...
impl WorkerRepository {
        ...
        if existing.is_some() {
            sqlx::query_as::<_, WorkerNode>(
                "UPDATE worker_nodes SET username = $1, ip = $2, cpu_cores = $3, memory_gb = $4,
                 ...
                 location = $12, status = $13, last_heartbeat = NOW(), updated_at = NOW()
                 WHERE worker_id = $14 RETURNING *",
            )
            .bind(&worker.username).bind(&worker.ip)
            .bind(worker.cpu_cores).bind(worker.memory_gb)
            .bind(worker.cpu_score).bind(worker.gpu_score).bind(worker.gpu_memory_gb)
            .bind(&worker.gpu_name).bind(worker.vram_mb)
            .bind(worker.storage_total_gb).bind(worker.storage_available_gb)
            .bind(&worker.location).bind(worker.status.as_str())
            ...
            .bind(&worker.worker_id).bind(&worker.username).bind(&worker.ip)
            .bind(worker.cpu_cores).bind(worker.memory_gb)
            .bind(worker.cpu_score).bind(worker.gpu_score).bind(worker.gpu_memory_gb)
            .bind(&worker.gpu_name).bind(worker.vram_mb)
            .bind(worker.storage_total_gb).bind(worker.storage_available_gb)
    ...
    pub async fn find_by_worker_id(&self, worker_id: &str) -> Result<Option<WorkerNode>> {
        sqlx::query_as::<_, WorkerNode>("SELECT * FROM worker_nodes WHERE worker_id = $1")
            .bind(worker_id).fetch_optional(&self.pool).await.map_err(Into::into)
    }
    ...
    pub async fn update_heartbeat(
        ...
        .bind(status).bind(cpu_usage).bind(memory_usage)
        .bind(gpu_usage).bind(gpu_memory_usage).bind(worker_id)
...
mod tests {
    ...
    async fn test_upsert_worker() {
        ...
        let worker = WorkerNode {
            id: uuid::Uuid::new_v4(), worker_id: "test-upsert-1".into(),
            username: "testuser".into(), ip: "192.168.1.1".into(),
            virtual_ip: None, hostname: Some("test-host".into()),
            cpu_cores: 4, memory_gb: 16, cpu_score: 100, gpu_score: 0, gpu_memory_gb: 0,
            gpu_name: Some("NVIDIA RTX 4060".into()), vram_mb: 8192,
            storage_total_gb: 500, storage_available_gb: 300,
            location: "us-east".into(), status: WorkerStatus::Active,
