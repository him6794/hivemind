...
- [ ] Fix Rust sandbox CLI argument compatibility: current args use `--memory-limit`, `--max-stack-depth`, and `--max-allocations`; `monty.exe --help` shows `--max-memory`, `--max-recursion-depth`, and `--max-allocations`.
- [ ] Fix `services/worker/examples/vpn_demo.go` build failure from obsolete `TaskResult.Output` usage.
