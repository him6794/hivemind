...
# 節點註冊（序號 1,2,3...）與狀態
next_node_id: int = 1
worker_to_node: dict[str, int] = {}
nodes: dict[int, dict] = {}
NODE_HEARTBEAT_TTL_SEC = 30
...
def _load_state() -> bool:
    ...
    global next_node_id, worker_to_node, nodes
    ...
    try:
        ...
        try:
            next_node_id = int(state.get('next_node_id') or 1)
        except Exception:
            next_node_id = 1
        worker_to_node = state.get('worker_to_node') or {}
        nodes = state.get('nodes') or {}
        # json 會把 key 變字串，這裡轉回 int
        try:
            nodes = {int(k): v for k, v in (nodes or {}).items()}
        except Exception:
            nodes = {}
...
def init_server_from_params(n):
    ...
    task_cursors = {}
    active_workers = set()
    # 每次 run 重新開始：節點仍保留註冊，但清空「目前任務/參與狀態」
    try:
        for nid, info in (nodes or {}).items():
            if isinstance(info, dict):
                info.pop('current_task_id', None)
                info.pop('current_task_progress', None)
                info.pop('current_task_range', None)
                info.pop('active_in_run', None)
                nodes[int(nid)] = info
    except Exception:
        pass
...
def _start_next_run_locked(depth: int = 0) -> bool:
    ...
    # 節點不夠就不啟動該 run：放回隊列最前面，等「alive 節點」數足夠再開始
    if desired_workers is not None and desired_workers > 0:
        now = time.time()
        alive = 0
        try:
            for _nid, info in (nodes or {}).items():
                if not isinstance(info, dict):
                    continue
                try:
                    if (now - float(info.get('last_seen_ts') or 0)) <= float(NODE_HEARTBEAT_TTL_SEC):
                        alive += 1
                except Exception:
                    # 若資料缺失，保守不算 alive
                    pass
        ...
        if alive < int(desired_workers):
            # 節點不足：跳過此任務，放到隊列最後，嘗試下一個
            job_queue.append(cfg)
            _save_queue()
            # 遞迴試下一個（depth+1 避免無限遞迴）
            return _start_next_run_locked(depth=depth + 1)
...
def register_node():
    ...
    with lock:
        ...
        # 已註冊則回同一個 node_id
        if worker_id in worker_to_node:
            nid = int(worker_to_node[worker_id])
            info = nodes.get(nid) or {}
            info['last_seen'] = _utc_iso()
            info['last_seen_ts'] = time.time()
            nodes[nid] = info
            _save_state()
            return jsonify({'status': 'ok', 'node_id': nid, 'max_active_workers': limit or 0, 'registered_nodes': len(worker_to_node)})

        global next_node_id
        nid = int(next_node_id)
        next_node_id = int(next_node_id) + 1
        worker_to_node[worker_id] = nid
        nodes[nid] = {
            'node_id': nid,
            'worker_id': worker_id,
...
def node_heartbeat():
    ...
    with lock:
        # 若未註冊，直接要求先註冊
        if worker_id not in worker_to_node:
            return jsonify({'error': 'not_registered'}), 409

        nid = int(worker_to_node[worker_id])
        if node_id is not None:
            try:
                if int(node_id) != nid:
                    return jsonify({'error': 'node_id_mismatch', 'expected': nid}), 409
            except Exception:
                pass

        info = nodes.get(nid) or {'node_id': nid, 'worker_id': worker_id}
        info['last_seen'] = _utc_iso()
        info['last_seen_ts'] = time.time()
        if isinstance(load, dict):
            info['load'] = load

        # 標記是否為本 run 允許的 active
        info['active_in_run'] = (worker_id in active_workers)
        nodes[nid] = info
        _save_state()
...
def list_nodes():
    with lock:
        ...
        for nid, info in (nodes or {}).items():
            ...
            alive = True
            try:
                alive = (now - float(last_seen_ts or 0)) <= float(NODE_HEARTBEAT_TTL_SEC)
            except Exception:
                alive = True

            task_elapsed = None
            try:
                start_ts = info.get('current_task_start_ts')
                if start_ts is not None:
                    task_elapsed = max(0.0, now - float(start_ts))
            except Exception:
                pass
            ...
            out.append({
                ...
                'active_in_run': bool(info.get('active_in_run')),
                'current_task_id': info.get('current_task_id'),
                'current_task_range': info.get('current_task_range'),
                'current_task_progress': info.get('current_task_progress'),
                'current_task_elapsed': task_elapsed,
            })

        out.sort(key=lambda x: int(x.get('node_id') or 0))
        limit = max_active_workers
        try:
        ...
        return jsonify({
            'nodes': out,
            'registered_nodes': len(worker_to_node),
            'alive_nodes': sum(1 for x in out if x.get('alive')),
            'max_active_workers': limit,
            'running': bool(task_running) or bool(in_flight) or bool(remaining_chunks),
            'current_run': current_run,
        })
...
@app.route('/setup_experiments', methods=['POST'])
def setup_experiments():
    ...
    - chunk_values: [50, 100, ...]
    - repeats: 50
    - clear_queue: true/false
    """
    data = request.json or {}
    
    n_values = data.get('n_values', [5, 6, 7, 8, 9, 10, 11, 12])
    worker_values = data.get('worker_values', [1, 10, 20, 30, 40])
    chunk_values = data.get('chunk_values', [50, 100, 150, 200, 250])
    repeats = int(data.get('repeats', 50))
    clear = bool(data.get('clear_queue', False))
    ...
    with lock:
                ...
                for chunks in chunk_values:
                    ...
                    for rep in range(1, repeats + 1):
                        job_queue.append({
                            'n': int(n),
                            'chunks': int(chunks),
                            'desired_workers': int(workers) if workers > 0 else None,
                            'rep_index': int(rep),
                            'rep_total': int(repeats),
        ...
        return jsonify({
            'status': 'ok',
            'combos': combos,
            'enqueued': len(job_queue) - initial_len,
            'total_queue': len(job_queue),
            'started_now': started,
            'n_values': n_values,
            'worker_values': worker_values,
            'chunk_values': chunk_values,
...
def experiment_results():
    ...
    - n: 過濾 10^n
    - chunks: 過濾 chunks
    - workers: 過濾 desired_workers
    - limit: 返回最多幾筆（默認500）
    - offset: 跳過前幾筆（默認0）
    """
    filter_n = request.args.get('n')
    filter_chunks = request.args.get('chunks')
    filter_workers = request.args.get('workers')
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if filter_n is not None:
                            try:
                                if int(item.get('n') or 0) != int(filter_n):
                                    continue
                            except Exception:
                                continue
                        if filter_chunks is not None:
                            try:
                                if int(item.get('chunks') or 0) != int(filter_chunks):
                                    continue
                            except Exception:
                                continue
                        if filter_workers is not None:
                            try:
                                if int(item.get('desired_workers') or 0) != int(filter_workers):
                                    continue
                            except Exception:
                                continue
    ...
    with lock:
        ...
        try:
            ...
            if nid is not None:
                ...
                info['last_seen'] = _utc_iso()
                info['last_seen_ts'] = time.time()
                info['active_in_run'] = (str(worker_id) in active_workers)
            ...
            try:
                ...
                if (not info) or (str(info.get('worker_id')) == str(worker_id)):
                    in_flight[task_id] = {
                        ...
                        'worker_id': worker_id,
                        'node_id': worker_to_node.get(str(worker_id)) if str(worker_id) in worker_to_node else node_id,
        ...
        try:
            nid = None
            if str(worker_id) in worker_to_node:
                nid = int(worker_to_node[str(worker_id)])
            elif node_id is not None:
                nid = int(node_id)
            if nid is not None:
                info = nodes.get(nid) or {'node_id': nid, 'worker_id': worker_id}
                ...
                info['current_task_range'] = f"{int(range_start)}-{int(range_end)}"
                # 用 cursor 推估進度
                try:
                    s = int(range_start)
                    e = int(range_end)
                    cur = int(task_cursors.get(task_id) or s)
                    pct = 100.0 if e <= s else max(0.0, min(100.0, (cur - s) / (e - s) * 100.0))
                except Exception:
                    pct = None
                info['current_task_progress'] = pct
                nodes[int(nid)] = info
        except Exception:
            pass
...
def get_task():
    ...
    with lock:
        ...
        # 要求先註冊節點序號（徹底控制）
        if str(worker_id) not in worker_to_node:
            return jsonify({'message': 'Node not registered'}), 204
        ...
        while remaining_chunks:
            ...
            if task_id:
                in_flight[task_id] = {
                    'assigned_at': now_ts,
                    'worker_id': worker_id,
                    'node_id': worker_to_node.get(str(worker_id)),
                    'task': task,
                }
            if start_time is None:
                start_time = now_ts
            # 更新節點目前領到的任務
            try:
                nid = int(worker_to_node.get(str(worker_id)))
                info = nodes.get(nid) or {'node_id': nid, 'worker_id': worker_id}
                info['last_seen'] = _utc_iso()
                info['last_seen_ts'] = now_ts
                info['active_in_run'] = (str(worker_id) in active_workers)
                info['current_task_id'] = task_id
                info['current_task_range'] = f"{int(task.get('start') or 0)}-{int(task.get('end') or 0)}"
                info['current_task_progress'] = None
                info['current_task_start_ts'] = now_ts
                nodes[int(nid)] = info
            except Exception:
                pass
            _save_state()
