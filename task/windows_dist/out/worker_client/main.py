...
def main():
    ...
    import socket
    # 讓 40 台分散機器下仍保持「穩定且唯一」：hostname + MAC(node id)
    # uuid.getnode() 在多數環境會回傳 MAC（或退化為隨機但仍相對唯一）
    worker_id = f"{socket.gethostname()}-{uuid.getnode():012x}"
    ...
    while True:
        try:
            ...
            print(f"\n完成！找到質數 {primes_count:,} 個，耗時 {duration:.2f}s (累計: {total_primes_found:,})")
